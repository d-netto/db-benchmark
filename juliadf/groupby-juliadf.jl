...
# if VERSION == v"1.6.1"
#     println("v1.6.1 GC stats"); flush(stdout);
# else
#     println("(fixed) jps/parallel-gc GC stats"); flush(stdout);
# end
...
for (bench_num, bt) in enumerate(bench_tasks_vec)
    ...
    #     stat = @timed bt.combine_task(x)

    #     @show stat.gctime, stat.time
    # end
    ...
    #TODO : 1. measure mark-time
    #       2. Use perf to profile gc-tim
    #       3. Measure "idle-time"
    #       4. Start thinking about stealinging
    while true
        stat = @timed begin
